...
```
*The client makes a request to the server. The server finds all of the saved users in database, then returns the users (JSON formatted) to the client with an OK.*
...
```
*The client makes a GET request to the server with the data of student_id of 1234567. The server takes the student_id=1234567 from that request and does a lookup in the database. The data has been found, then the server returns the user (JSON formatted) to the client with an OK.*
3 **GET /user?student_id=0** - *Get the infomation of the user with the student_id of 0*
```
Response :
{
...
4 **POST /user** - *Attempt to create an user with the student id that has already been used*
```
Request :
{
    "student_id": "1234567",
...
5 **POST /user** - *Create an user and store it in the database*
```
Request:
{
...
```
*The client makes a POST request with the data of ^. The server validates the request body, then takes the student_id that from that request and does a lookup in the database. The server fails to find any data with that student_id, then the server saves the user to the database and returns what have been saved to the client with the status of OK.*
6 **GET /user?student_id={student_id}** - *Get the infomation of the user with the student_id of {student_id}*
```
Response :
{
    "student_id": "<STUDENT_ID>",
...
```
7 **GET /github/{studentId}** - *Attempt to redirect to user's github account, but the user doesn't have github information in the profile* 
```
...
```
8 **PUT /user** - *Update the user information*
```
Request:
{
  "student_id": "<STUDENT_ID>",
...
```
9 **GET /github/{studentId}** - *Redirect to user's github profile*
```
magics.
```
10 **DELETE  /user** - *Delete a user from database*
```
Request :
{
  "student_id": "<STUDENT_ID>"
...
```
11 **GET /user/null** - *Internal Server Error* 
```
Response :
{
    "message": "/ by zero",
